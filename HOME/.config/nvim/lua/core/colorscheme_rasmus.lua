...
local colors = function(variant)
  if variant == "monochrome" then
    return {
      -- Basic
      bg = "#1a1a19",
      bg_nc = "#1a1a19",
...
M.setup = function()
  ...
  local groups = {
    ...
    NormalNC = { fg = c.fg, bg = cfg.transparent and c.none or c.bg_nc }, -- normal text in non-current windows
    SignColumn = { fg = c.fg, bg = c.none }, -- column where signs are displayed
    EndOfBuffer = { fg = c.gray03 }, -- ~ lines at the end of a buffer
    NormalFloat = { fg = get_highlight("FloatBorder", "foreground"), bg = get_highlight("Normal", "background") }, -- normal text and background color for floating windows
