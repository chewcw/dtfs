...
vim.api.nvim_create_user_command("UrlEncode", function()
  ...
  local replacement_text = require("core.utils").url_encode(text)
  local replacement_lines = (replacement_text == "") and { "" } or vim.split(replacement_text, "\n", { plain = true })
  vim.api.nvim_buf_set_text(buf, start_row, start_col, end_row, end_col, replacement_lines)
...
vim.api.nvim_create_user_command("UrlDecode", function()
  ...
  local replacement_text = require("core.utils").url_decode(text)
  local replacement_lines = (replacement_text == "") and { "" } or vim.split(replacement_text, "\n", { plain = true })
  vim.api.nvim_buf_set_text(buf, start_row, start_col, end_row, end_col, replacement_lines)
end, { nargs = 0, range = true })
