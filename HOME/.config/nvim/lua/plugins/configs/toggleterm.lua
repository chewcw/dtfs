local options = {
  ...
  on_exit = function(term, job, exit_code, name)
    ...
    if utils.table_contains(x, term.id) then
      -- table.remove takes an index, not a value; find the index first
      for i, opened_term_id in ipairs(x) do
        if opened_term_id == term.id then
          table.remove(x, i)
          break
        end
      end
    ...
    -- pick the first remaining id for the toggle_term_count record,
    ...
    if vim.g.toggle_term_saved_list ~= nil then
