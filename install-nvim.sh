...
install_nvim_v010() {
  if ! command -v nvim &>/dev/null; then
    ...
    sudo cp -r /tmp/nvim-linux64 /usr/local/nvim-v0-10-1
    sudo ln -sf /usr/local/nvim-v0-10-1/bin/nvim /usr/bin/nvim
  fi
  echo "nvim v0.10.1 is installed"
}
